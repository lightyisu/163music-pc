import { getBanner, getHotRecommend, getNewAlbum, getSettleSinger, getSongDetail, getTopranking, getLyric } from '../service/request';
import { CHANGE_BANNERS, CHANGE_CURRENT_SONG, CHANGE_HOT_RECOMMEND, CHANGE_HOT_STREAMER, CHANGE_LYRIC, CHANGE_NEW_ALBUM, CHANGE_PLAYLIST, CHANGE_PLAY_SEQUENCE, CHANGE_SETTLE_SINGER, CHANGE_SONG_INDEX, CHANGE_TOP_RANKING } from '../constants/actionType';
...
const getSongDetailAction = (id) => {
    ...
    return (dispatch, getState) => {
        ...
        if (songIndex !== -1) {
            ...
            Promise.all([getSongDetail(id), getLyric(id)]).then((res) => {
                const newPlaylist = [...playlist];
                newPlaylist.push({ ...res[0].songs[0] });
                dispatch({ type: CHANGE_LYRIC, payload: parseLyric(res[1].lrc.lyric) })
                dispatch({ type: CHANGE_PLAYLIST, payload: newPlaylist })
                dispatch({ type: CHANGE_SONG_INDEX, payload: newPlaylist.length - 1 })
                dispatch({ type: CHANGE_CURRENT_SONG, payload: newPlaylist[newPlaylist.length - 1] })
            })
...
const changeCurrentSongAction = (song) => ({ type: CHANGE_CURRENT_SONG, payload: song })


export { getBannerAction, getHotRecommendAction, getNewAlbumAction, getToprankingAction, getSettleSingerAction, getHotStreamerAction, getSongDetailAction, changePlaySequenceAction, changeCurrentSongAction }